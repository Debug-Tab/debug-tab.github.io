...
function pandown() {
    ...
    for (var i = 0; i < pandowns.length; i++) {
		...
		pandowns[i].innerHTML = `<div><p class="pan-fname" style="color: 
		${colors[type]}">${pandowns[i].getAttribute('fname')}</p><br><stong class="pan-fintroduce">${names[type]}链接：${pandowns[i].getAttribute('url')} 提取码：${pandowns[i].getAttribute('pwd')}</stong><br></div><div><button class="pan-down" style="background-color: ${colors[type]}" title="复制提取码并跳转至下载页面" onclick="copyText('${pandowns[i].getAttribute('pwd')}', '${pandowns[i].getAttribute('url')}');">复制提取码并下载</button></div>`;
    }
}
